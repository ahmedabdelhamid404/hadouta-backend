...
async function main(): Promise<void> {
  ...
  console.log("\n→ Generating Pixar cover (iteration 6 — Nano Banana 2)...");
  const coverResult = await generateCoverIllustration({
    ...
    // No provider — defaults to "nano-banana" which now points to Nano Banana 2.
  });
  ...
  for (const pageNum of SELECTED_BODY_PAGES) {
    ...
    console.log(`\n→ Generating Pixar page ${pageNum} (iteration 6 — Nano Banana 2)...`);
