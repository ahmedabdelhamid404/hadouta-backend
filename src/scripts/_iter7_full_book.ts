// ITER 7 — FULL BOOK: cover + all 16 body pages with all stacked fixes:
//   ✓ Aspect ratio 3:4 (portrait, fits A5 PDF — no cropping)
//   ✓ Cover scale fix (protagonist at normal human scale, not monster-sized)
//   ✓ Image 1 = IDENTITY REFERENCE only (face/skin/hair, ignore expression)
//   ✓ Image 2 = STYLE REFERENCE only (watercolor medium, ignore character)
//   ✓ Generate-from-scratch framing (not edit)
//   ✓ Per-beat positive face/expression narrative (no smile default)
//   ✓ Multi-turn refinement (turn 1 + turn 2 self-critique)
//   ✓ thought_signature pass-through (raw model parts)
//   ✓ Unlimited 5-min retry on Google 503/429/500
//   ✓ Resumable: re-run picks up where it stopped
//   ✓ Cloudinary upload retry (3 attempts)
//
// Run fresh:    pnpm tsx src/scripts/_iter7_full_book.ts
// Resume:       pnpm tsx src/scripts/_iter7_full_book.ts <gen_id>
...
const SOURCE_GEN_ID = "68d5add6-48da-4a3e-baf3-054ad2162326"; // watercolor baseline (story + bible + 16-page sources)
const ORDER_ID = "76e6226a-452e-47d6-9209-b53717d6d1cd";
const PRIMARY_MODEL = "gemini-3.1-flash-image-preview"; // Nano Banana 2 (Flash) — back to original per founder direction
const ASPECT_RATIO = "3:4"; // portrait, fits A5 PDF assembly without cropping
const ALL_PAGES = Array.from({ length: 16 }, (_, i) => i + 1); // pages 1..16
...
async function callGoogleApi(contents: ContentTurn[]): Promise<ApiResult> {
  ...
  while (true) {
    ...
    try { return await callGoogleApiOnce(contents); }
      ...
      console.log(`      retry in 1min (attempt ${attempt + 1})...`);
...
function sceneNarrativeFromBeat(beat: string, charactersOnPage: string[], protagonist: string): string {
  ...
  if (beatLower.includes("joy") || beatLower.includes("happ") || beatLower.includes("celebrat"))
    return `${protagonist} is in three-quarter view, eyes bright and crinkled with genuine joy reacting to the scene around her. Her mouth is open in a real, unposed laugh — the eyes lead the joy, the smile follows from the eyes.${otherClause}`;
...
function buildPrompt(args: {
  ...
  const sceneNarrative = args.isCover
    ? `${args.childName} is rendered as the iconic cover character at NORMAL human scale relative to her surroundings — her body, head, and face are proportional to a real 5-year-old child, not enlarged or shrunk relative to background elements or other characters. She is in three-quarter view with her face turned slightly toward the viewer, a warm natural expression — eyes open and engaged, lips softly closed with corners just gently lifted in a quiet, real smile (not a posed-for-camera grin).`
...
ASPECT RATIO: 3:4 portrait (taller than wide). The image is fixed at this exact ratio — compose elements to fill this frame; nothing important should be cropped at edges.

SCALE: all characters appear at REALISTIC RELATIVE PROPORTIONS — a 5-year-old child is shorter than an adult, similar height to other 5-year-olds, and small relative to architectural elements (doors, walls, furniture). DO NOT render the protagonist as oversized vs background or other characters; DO NOT render other characters as tiny vs the protagonist.

Composition: ${args.childName}'s face fully readable — three-quarter view minimum, both eyes and nose visible. She fills approximately 40-60% of frame height. Face anchored at a rule-of-thirds intersection.
...
Re-render the same scene with these specific corrections to make ${childName} more clearly recognizable: pull her eye shape closer to image 1's exact shape, pull her iris color closer to image 1's exact color, restore image 1's exact hair texture and curl pattern, match image 1's exact skin tone, restore image 1's specific jaw and chin shape (do not soften toward generic round). Also verify scale — all characters must appear at realistic relative proportions, no oversized protagonist.

Keep the same scene, same pose narrative, same composition, same watercolor style, same 3:4 aspect ratio. Only refine ${childName}'s face geometry and verify scale.`;
...
async function main(): Promise<void> {
  console.log(`Iter 7 — FULL BOOK (cover + ${ALL_PAGES.length} pages) | aspect ${ASPECT_RATIO} | scale-fix\n`);
  ...
  // Cover
  if (!haveCover) {
    console.log(`\n→ Cover (3:4 + scale-fix + multi-turn)...`);
    ...
    const uploaded = await uploadWithRetry(buf, "illustration_cover_iter7", generated.mimeType, "cover");
  ...
  // For pages without a watercolor source, fall back to using the iter 7 cover
  // as the style reference (it's already in the target watercolor style).
  const iterCoverUrl = (await db.select().from(generations).where(eq(generations.id, iterGenId)).limit(1).then((r) => r[0]))?.coverUrl;
  if (!iterCoverUrl) throw new Error("iter cover URL missing — cannot fall back");
  ...
  for (const pageNum of ALL_PAGES) {
    ...
    // Use page-specific watercolor source if available, otherwise use iter cover
    ...
    console.log(`\n→ Page ${pageNum}/${ALL_PAGES.length} (3:4) | beat: "${storyPage.emotionalBeat}" | style-ref: ${styleRefSrc}`);
    const illustration = await fetchAsBase64(styleRefUrl);
    const generated = await multiTurnRefine({
      childName: "Hena",
      isCover: false,
    ...
    const buf = Buffer.from(generated.data, "base64");
    const uploaded = await uploadWithRetry(buf, `illustration_page_${pageNum}_iter7`, generated.mimeType, `page${pageNum}`);
    console.log(`  ✓ ${uploaded.url}`);
    await db.insert(bookPages).values({
      ...
      illustrationProvider: "gemini-3.1-iter7-full",
  ...
  console.log(`   To resume if interrupted: pnpm tsx src/scripts/_iter7_full_book.ts ${iterGenId}`);
}
