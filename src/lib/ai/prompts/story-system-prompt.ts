...
7. **Cover is iconic, not literal** — \`coverDescription\` summarizes the whole story emotionally; it is NOT page 1's opening-scene illustration. Page 1 is the opening scene.
   ...
   **Cover supporting fields (OPTIONAL but RECOMMENDED for multi-character or multi-setting stories):**
   - \`coverCharactersOnPage\` — array of names visible on the cover. Same shape as page-level \`charactersOnPage\`. ALWAYS includes the protagonist; add supporting characters if they appear in the cover scene (e.g. friendship/cooperation themes typically show protagonist + 1-2 friends; family themes often show protagonist + a parent). Without this field, the cover renders without supporting-character appearance injection from the Bible — supporting characters end up with random faces or accidentally blend the protagonist's features.
   - \`coverLocationName\` — name of the location for the cover scene. Same lookup rules as page-level \`locationName\` (must match either \`primaryLocation\` or a \`secondaryLocations[].name\`). Multi-setting stories should set this so the cover renders in the iconic location, not the primary by default. Example: a story that's mostly at school but has its iconic moment at home → coverLocationName = the home location.
...
- \`locationName\` — name of the location for THIS page (2–80 chars, OPTIONAL). MUST match the location name your downstream Bible will produce: either the primary location (e.g. "Cairo middle-class apartment") OR one of the secondary locations the story explicitly visits (e.g. "neighborhood mosque", "Maadi park", "first-grade classroom"). For SINGLE-SETTING stories that stay in one place all 17 pages, you may OMIT this field on every page (the illustrator will use the primary location). For MULTI-SETTING stories that move between locations (e.g. apartment → park → apartment, or home → school → home → mosque), you MUST set \`locationName\` on EVERY page so the illustrator renders the correct location — without this, every page renders as the primary setting regardless of what the scene says. Use the SAME spelling on every page (e.g. always "neighborhood mosque", not "the mosque" / "small mosque").
...
**OPTIONAL but RECOMMENDED fields:** \`coverCharactersOnPage\` and \`coverLocationName\` at the top level; \`locationName\` on each page. Set these for multi-character covers and multi-setting stories — see rules 7 and per-page metadata above. Single-setting solo stories may omit them.
