...
## Style anchor — locked soft watercolor register

The brand is committed to a single visual register: **soft Egyptian children's-book watercolor — wet-on-wet technique, visible brush strokes, pigment blooms, cold-press paper texture, the soft watercolor warmth of Tomie dePaola's _Strega Nona_ and Helen Oxenbury's _We're Going on a Bear Hunt_, applied to Egyptian children and Cairo apartment settings**. Per Google's official Gemini image-prompting guidance, the canonical style-transfer pattern is "in the style of [named artist]'s [specific named work]" — these two book references are the recommended primary anchors for soft watercolor on Nano Banana 2.

Use natural-language descriptive sentences (NOT comma-separated tag lists — the production illustration model is Nano Banana 2 / Gemini 3.1 Flash Image, an autoregressive multimodal model that ignores tag soup and weighted-token syntax). The two load-bearing activators that reliably trigger the watercolor register on Gemini are **\`wet-on-wet\`** and **\`visible brush strokes\`** — confirmed across every Google Cloud, Google Developers, and community guide source on Gemini watercolor prompting (2025-2026).

Your styleBible block must look like this:

- **medium**: "Watercolor children's-book illustration, wet-on-wet technique, visible brush strokes and pigment blooms, soft bleeding edges where colors meet, cold-press paper texture showing through warm cream washes — the soft watercolor register of Tomie dePaola's _Strega Nona_ and Helen Oxenbury's _We're Going on a Bear Hunt_, applied to Egyptian children and Cairo apartment settings (terracotta tiles, mashrabiya screens, warm Mediterranean light, NOT New England farmhouse register). Gentle pencil under-drawing visible at edges, no hard digital lines."
- **palette**: "Warm cream paper backgrounds, terracotta and ochre accents, soft sage greens for foliage, dusty blues for sky, golden afternoon light suffusing the scene; muted earth tones overall, never neon or oversaturated; keep generous areas of warm cream paper showing through (paper white preserved)."
- **light**: "Warm golden afternoon light filtering through soft window curtains; gentle directional lighting with soft falloff and luminous edges; ambient watercolor luminosity (the paper itself glows through the washes)."
- **negativeStyle**: KEEP THIS SHORT — ≤75 tokens, 3–5 items max. Long negative lists actively HURT Nano Banana 2's reasoning planner (every item adds weight to its interpretation). Use this exact pattern: "NOT photorealistic, NOT 3D-rendered, NOT digital-glossy, NOT vector-flat, NOT sharp digital lines, NOT a photo-filter watercolor over a photo — soft hand-painted watercolor storybook only. No text, letters, numbers, or typography anywhere in the image."
- **compositionAnchors**: "Protagonist occupies approximately 60% of frame height; setting fills the remaining 40% as supporting context. Face clearly readable at thumbnail size. Rule-of-thirds anchoring for the protagonist's face on identity-critical pages — face placed at one of the four intersection points, not dead-center. **Facial-geometry hard constraint: preserve eye spacing, nose width, jaw shape, and any dimple/freckle pattern from the reference photo — only the surface treatment should become painterly, not the underlying face structure. Eyes painted with crisper edges than the rest of the face (they are the identity anchor).** Cover composition: subject in upper two-thirds of frame; bottom one-third neutral (the PDF cover layout fades the bottom edge into cream paper)."

CRITICAL rules for this register (violations break the illustration pipeline):

- **Always include \`wet-on-wet\` alongside \`visible brush strokes\` in \`medium\`** — these two are the canonical Gemini watercolor activators per Google Cloud's Nano Banana prompting guide. Omitting either degrades watercolor fidelity measurably.
- **Always reference the canonical book pattern \`"in the style of [artist]'s [specific named work]"\`** when invoking artist style. Tomie dePaola's _Strega Nona_ and Helen Oxenbury's _We're Going on a Bear Hunt_ are the recommended primary anchors for soft watercolor on Nano Banana 2. Generic descriptors like "children's book illustration" are weaker than named-work references.
- **Always redirect the Western-canon artist references toward Egyptian content** in \`medium\` — phrase like "applied to Egyptian children and Cairo apartment settings, NOT New England farmhouse register" — otherwise the artist references bring Western cultural baggage that fights Hadouta's cultural-specificity moat (ADR-002 + ADR-020).
- **Never** include "Pixar", "Disney", "3D-rendered", "CGI", "subsurface scattering", "smooth digital", "rendered", or "stylized 3D" in the positive style fields — those terms pull Nano Banana 2 toward 3D-animated register and produce hybrid "watercolor-style 3D rendering" failures (the model meeting prompt halfway).
- **Never** add hazy/dreamy/ethereal language to the watercolor anchors — those keywords degrade face-fidelity directly by softening high-frequency facial details past the recognition threshold.
...
- **Use positive framing in \`light\` and \`palette\` fields** — describe what the rendering SHOULD have, never "no harsh shadows" / "no studio lighting" (those belong in \`negativeStyle\` only). Per Google's official Gemini prompting guidance.
